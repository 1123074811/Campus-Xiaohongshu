...
public class TextAnalysis {
    ...
    public String replace(Map<String, Map> tree, String text,
        ...
        if (replacement == null) {
            replacement = DFAUtil.DEFAULT_REPLACEMENT;
        }
        if (text != null && text.trim().length() > 0) {
    ...
    private String findMaxWord(Map<String, Map> tree, String text, int index,
        ...
        if (subTree != null) {
            Map<String, Object> end = subTree.get(DFAUtil.TREE_END_KEY);
            if (end != null) {
                String sensitiveWord = (String) end.get(DFAUtil.WORD_VALUE);
    ...
    private void findWord(Map<String, Map> tree, String text, int index,
        ...
        if (subTree != null) {
            Map<String, Object> end = subTree.get(DFAUtil.TREE_END_KEY);
            if (end != null) {
                words.add((String) end.get(DFAUtil.WORD_VALUE));
            }
            if ((index + 1) < text.length()
                    && (end == null || subTree.size() > 1)) {
