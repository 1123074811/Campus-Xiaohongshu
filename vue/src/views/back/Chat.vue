...
const save = () => {
  ...
  request.post("/chat", form.value).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete("/chat/" + id).then(res => {
    if (res.code === '200') {
...
const delBatch = () => {
  ...
  request.post("/chat/del/batch", ids).then(res => {
    if (res.code === '200') {
  ...
  <div class="content-container">
    ...
    <el-card>
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="文本类型" width="100" align="center">
        ...
        <el-table-column label="发往用户" width="100" align="center">
          <template #default="scope">
        ...
        <el-table-column label="是否已读" width="100" align="center">
          <template #default="scope">
            <el-switch v-model="scope.row.isRead" @change="changeState(scope.row)"/>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id ? '编辑' : '新增'" width="30%" center>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="发往用户">
          <el-select v-model="form.toUserId" placeholder="请选择发往用户" style="width: 220px">
            <el-option v-for="item in users" :key="item.id" :label="item.nickname" :value="item.id" />
          </el-select>
        </el-form-item>
