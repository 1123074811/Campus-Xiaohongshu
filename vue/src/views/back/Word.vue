...
// 保存
const save = () => {
  request.post("/word", form.value).then(res => {
    if (res.code === '200') {
...
const delBatch = () => {
  ...
  request.post("/word/del/batch", ids).then(res => {
    if (res.code === '200') {
...
<template>
  <div class="content-container">
    ...
    <div class="header-section">
      <el-input v-model="searchForm.keyword" placeholder="请输入敏感词" class="filter-input" :prefix-icon="Search" clearable/>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id ? '编辑' : '新增'" width="30%" destroy-on-close center>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="说明" required>
          <el-input type="textarea" v-model="form.info" placeholder="请输入说明" />
        </el-form-item>
      </el-form>
      <template #footer>
